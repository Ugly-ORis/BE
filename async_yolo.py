...
sealing_roi = ((510, 300), (630, 410))  # 실링 검출용 ROI
frame_interval = 0.1  # 0.1초 간격으로 추론
last_time = time.time()
last_state = {"sealing_detected": False, "distance_state": None}
...
def execute_tts(message):
    ...
    tts.save("text_tts.mp3")
    os.system("mplayer text_tts.mp3")
    os.system("rm text_tts.mp3")
    ...
    while cap.isOpened():
        ...
        if time.time() - last_time >= frame_interval:
            ...
            if results and results[0].masks:
                detected_objects = {"person": [], "robot_arm": [], "sealing": [], "holder": [], "ice_cream": []}
                for i, mask in enumerate(results[0].masks.xy):
                    ...
                    if label in detected_objects:
                        ...
                        color = {
                            ...
                            "sealing": (0, 0, 255), "holder": (255, 255, 0),
                            "ice_cream": (255, 0, 255)
                ...
                sealing_detected = any(
                    check_jig_position(mask_points, sealing_roi)
                    for mask_points in detected_objects["sealing"]
                )
                if sealing_detected:
                    print("실링 검출: 실링을 제거해주세요!")
                    await async_speak("실링을 제거해주세요!", "sealing_detected")
                else:
                    last_state["sealing_detected"] = False
                ...
                (x1, y1), (x2, y2) = sealing_roi
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 255), 2)
                cv2.putText(frame, "sealing ROI", (x1, y1 - 10),
